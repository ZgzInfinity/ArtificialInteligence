...
(defrule CP::valor-asignado-caja
	(celda (id ?id)(caja ?caja)(rango ?valor))
	?celda <- (celda (id ~?id)(caja ?caja)(rango $?principio ?valor $?final)
					(acciones $?acciones))
=>
	(modify ?celda (rango ?principio ?final)
...
(defrule CP::pares-asignados-fila
	(celda (fila ?fila) (columna ?columna1) (rango ?valor1 ?valor2))
	(celda (fila ?fila) (columna ?columna2&~?columna1) (rango ?valor1 ?valor2))
	?celda <- (celda (fila ?fila)(columna ~?columna2&~?columna1)(rango $?principio ?valor1 $?medio ?valor2 $?final)
				(acciones $?acciones))
=>
	(modify ?celda (rango ?principio ?medio ?final)
     	(acciones ?acciones (create$ pares-asignados-fila ?celda ?valor1 ?valor2)))
	
	
   ) 

 
(defrule CP::pares-asignados-columna
	(celda (fila ?fila1) (columna ?columna) (rango ?valor1 ?valor2))
	(celda (fila ?fila2&~?fila1) (columna columna)(rango ?valor1 ?valor2))
	?celda <- (celda (fila ~?fila2&~?fila1)(columna ?columna)(rango $?principio ?valor1 $?medio ?valor2 $?final)
				(acciones $?acciones))
=>
	(modify ?celda (rango ?principio ?medio ?final)
     	(acciones ?acciones (create$ pares-asignados-columna ?celda ?valor1 ?valor2)))
	
	
   ) 

(defrule CP::pares-asignados-caja
	(celda (id ?id1) (caja ?caja) (rango ?valor1 ?valor2))
	(celda (id ?id2&~?id1) (caja ?caja)(rango ?valor1 ?valor2))
	?celda <- (celda (id ~?id2&~?id1)(caja ?caja)(rango $?principio ?valor1 $?medio ?valor2 $?final)
				(acciones $?acciones))
=>
	(modify ?celda (rango ?principio ?medio ?final)
     	(acciones ?acciones (create$ pares-asignados-caja ?celda ?valor1 ?valor2)))
...
(defrule CP::valor-oculto-fila
	?celda <- (celda (fila ?fila) (columna ?columna)(rango $?principio ?valor $?final&:(> (+ (length$ ?principio) (length$ ?final)) 0))
						(acciones $?acciones))
	(not (celda (fila ?fila) (columna ~?columna) (rango $? ?valor $?)))
=>
...
(defrule CP::valor-oculto-caja
	?celda <- (celda (id ?id) (caja ?caja)(rango $?principio ?valor $?final&:(> (+ (length$ ?principio) (length$ ?final)) 0))
						(acciones $?acciones))
	(not (celda (id ~?id) (caja ?caja) (rango $? ?valor $?)))
=>
	(modify ?celda (rango ?valor)
...
(defrule CP::pares-ocultos-fila
	?celda1 <- (celda (fila ?fila) (columna ?columna1)(rango $?principio ?valor1 $?medio ?valor2 $?final&:(> (+ (length$ $?principio) (+ (length$ $?medio) (length$ $?final))) 0))
						(acciones $?acciones1))
	?celda2 <- (celda (fila ?fila) (columna ?columna2&~?columna1)(rango $?principio2 ?valor1 $?medio2 ?valor2 $?final2&:(> (+ (length$ $?principio2) (+ (length$ $?medio2) (length$ $?final2))) 0))
						(acciones $?acciones2))
	(not (celda (fila ?fila) (columna ~?columna2&~?columna1) (rango $? ?valor1|?valor2 $?)))	
=>
	(modify ?celda1 (rango ?valor1 ?valor2)
     	(acciones ?acciones1 (create$ pares-oculto-fila ?celda1 ?valor1 ?valor2)))
	(modify ?celda2 (rango ?valor1 ?valor2)
     	(acciones ?acciones2 (create$ pares-ocultos-fila ?celda2 ?valor1 ?valor2)))
	
	)


(defrule CP::pares-ocultos-columna
	?celda1 <- (celda (fila ?fila1) (columna ?columna)(rango $?principio ?valor1 $?medio ?valor2 $?final&:(> (+ (length$ $?principio) (+ (length$ $?medio) (length$ $?final))) 0))
						(acciones $?acciones1))
	?celda2 <- (celda (fila ?fila2&~?fila1) (columna ?columna)(rango $?principio2 ?valor1 $?medio2 ?valor2 $?final2&:(> (+ (length$ $?principio2) (+ (length$ $?medio2) (length$ $?final2))) 0))
						(acciones $?acciones2))
	(not (celda (fila ~?fila2&~?fila1) (columna ?columna) (rango $? ?valor1|?valor2 $?)))	
=>
	(modify ?celda1 (rango ?valor1 ?valor2)
     	(acciones ?acciones1 (create$ pares-oculto-columna ?celda1 ?valor1 ?valor2)))
	(modify ?celda2 (rango ?valor1 ?valor2)
     	(acciones ?acciones2 (create$ pares-ocultos-columna ?celda2 ?valor1 ?valor2)))
		
	)
	

(defrule CP::pares-ocultos-caja
	?celda1 <- (celda (id ?id1) (caja ?caja)(rango $?principio ?valor1 $?medio ?valor2 $?final&:(> (+ (length$ ?principio) (+ (length$ $?medio) (length$ $?final))) 0))
						(acciones $?acciones1))
	?celda2 <- (celda (columna ?id2&~?id1) (caja ?caja)(rango $?principio2 ?valor1 $?medio2 ?valor2 $?final2&:(> (+ (length$ $?principio2) (+ (length$ $?medio2) (length$ $?final2))) 0))
						(acciones $?acciones2))
	(not (celda (id ~?id2&~?id1) (caja ?caja) (rango $? ?valor1|?valor2 $?)))	
=>
	(modify ?celda1 (rango ?valor1 ?valor2)
     	(acciones ?acciones1 (create$ pares-oculto-caja ?celda1 ?valor1 ?valor2)))
	(modify ?celda2 (rango ?valor1 ?valor2)
     	(acciones ?acciones2 (create$ pares-ocultos-caja ?celda2 ?valor1 ?valor2)))
...
(defrule CP::interseccion-fila-caja
	(celda (id ?id1)(fila ?fila) (caja ?caja) (rango $?principio ?valor $?final&:(> (+ (length$ $?principio) (length$ $?final)) 0)))
	(not (celda (fila ?fila)(caja ~?caja)(rango $?principio2 ?valor $?final2)))
	?celda <- (celda (fila ~?fila) (caja ?caja) (rango $?principio2 ?valor $?final2) (id ~?id1)(acciones $?acciones))
=>
	(modify ?celda (rango $?principio2 $?final2)
...
(defrule CP::interseccion-columna-caja
	(celda (id ?id1)(columna ?columna) (caja ?caja) (rango $?principio ?valor $?final&:(> (+ (length$ $?principio) (length$ $?final)) 0)))
	(not (celda (columna ?columna)(caja ~?caja)(rango $?principio2 ?valor $?final2)))
	?celda <- (celda (columna ~?columna) (caja ?caja) (rango $?principio2 ?valor $?final2) (id ~?id1)(acciones $?acciones))
=>
	(modify ?celda (rango $?principio2 $?final2)
...
;	(celda (fila ?fila)(columna ?columna)(id ?id)(rango $?principio ?valor $?final))
;	(celda (fila ?fila)(columna ?columna2&~?columna)(id ?id2&~?id)(rango $? principio2 ?valor $?final2))
;	(celda (fila ?fila2&~?fila)(columna ?columna)(id ?id3&~?id2&~id)(rango $?principio3 ?valor $?final3))
;	(celda (fila ?fila2)(columna ?columna2)(id ?id4&~?id3&~?id2&~id)(rango $?principio4 ?valor $?final4))
;	(not (celda (fila ?fila) (columna ~?columna2&~?columna1) (rango $? ?valor $?)))
;	?celda <- (celda (columna ?columna|?columna2)(id ?id5&~?id4&~?id3&~?id2&~id)(rango $?principio5 ?valor $?final5&:(> (+ (length$ ?principio5) (length$ ?final5)) 0)))
;	(not (celda (fila ?fila|?fila2)(id ?id6&~?id5&~?id4&~?id3&~?id2&~id)(rango $?principio6 ?valor $?final6)))
;=>
;	(modify ?celda (rango ?principio5 ?final5)(acciones $?acciones 
;	(create$ cruz-fila-columna ?celda ?valor)))
;	
;	)


;(defrule CP::cruz-fila-caja
;	(celda (fila ?fila)(caja ?caja)(id ?id)(rango $?principio ?valor $?final))
;	(celda (fila ?fila)(caja ?caja2&~?caja)(id ?id2&~?id)(rango $? principio2 ?valor $?final2))
;	(celda (fila ?fila2&~?fila)(caja ?caja)(id ?id3&~?id2&~id)(rango $?principio3 ?valor $?final3))
;	(celda (fila ?fila2)(caja ?caja2)(id ?id4&~?id3&~?id2&~id)(rango $?principio4 ?valor $?final4))
;	(not (celda (fila ?fila) (caja ~?caja2&~?caja1) (rango $? ?valor $?)))
;	?celda <- (celda (caja ?caja|?caja2)(id ?id5&~?id4&~?id3&~?id2&~id)(rango $?principio5 ?valor $?final5&:(> (+ (length$ ?principio5) (length$ ?final5)) 0)))
;	(not (celda (fila ?fila|?fila2)(id ?id6&~?id5&~?id4&~?id3&~?id2&~id)(rango $?principio6 ?valor $?final6)))
;=>
;	(modify ?celda (rango ?principio5 ?final5)(acciones $?acciones 
;	(create$ cruz-fila-caja ?celda ?valor)))
;	
;	)


;(defrule CP::cruz-columna-fila
;	(celda (columna ?columna)(fila ?fila)(id ?id)(rango $?principio ?valor $?final))
;	(celda (columna ?columna)(fila ?fila2&~?fila)(id ?id2&~?id)(rango $? principio2 ?valor $?final2))
;	(celda (columna ?columna2&~?columna)(fila ?fila)(id ?id3&~?id2&~id)(rango $?principio3 ?valor $?final3))
;	(celda (columna ?columna2)(fila ?fila2)(id ?id4&~?id3&~?id2&~id)(rango $?principio4 ?valor $?final4))
;	(not (celda (columna ?columna) (fila ~?fila2&~?fila1) (rango $? ?valor $?)))
;	?celda <- (celda (fila ?fila|?fila2)(id ?id5&~?id4&~?id3&~?id2&~id)(rango $?principio5 ?valor $?final5&:(> (+ (length$ ?principio5) (length$ ?final5)) 0)))
;	(not (celda (columna ?columna|?columna2)(id ?id6&~?id5&~?id4&~?id3&~?id2&~id)(rango $?principio6 ?valor $?final6)))
;=>
;	(modify ?celda (rango ?principio5 ?final5)(acciones $?acciones 
;	(create$ cruz-columna-fila ?celda ?valor)))
;	
;	)


;(defrule CP::cruz-columna-caja
;	(celda (columna ?columna)(caja ?caja)(id ?id)(rango $?principio ?valor $?final))
;	(celda (columna ?columna)(caja ?caja2&~?caja)(id ?id2&~?id)(rango $? principio2 ?valor $?final2))
;	(celda (columna ?columna2&~?columna)(caja ?caja)(id ?id3&~?id2&~id)(rango $?principio3 ?valor $?final3))
;	(celda (columna ?columna2)(caja ?caja2)(id ?id4&~?id3&~?id2&~id)(rango $?principio4 ?valor $?final4))
;	(not (celda (columna ?columna) (caja ~?caja2&~?caja1) (rango $? ?valor $?)))
;	?celda <- (celda (caja ?caja|?caja2)(id ?id5&~?id4&~?id3&~?id2&~id)(rango $?principio5 ?valor $?final5&:(> (+ (length$ ?principio5) (length$ ?final5)) 0)))
;	(not (celda (columna ?columna|?columna2)(id ?id6&~?id5&~?id4&~?id3&~?id2&~id)(rango $?principio6 ?valor $?final6)))
;=>
;	(modify ?celda (rango ?principio5 ?final5)(acciones $?acciones 
;	(create$ cruz-columna-caja ?celda ?valor)))
;	
;	)


;(defrule CP::cruz-caja-fila
;	(celda (caja ?caja)(fila ?fila)(id ?id)(rango $?principio ?valor $?final))
;	(celda (caja ?caja)(fila ?fila2&~?fila)(id ?id2&~?id)(rango $? principio2 ?valor $?final2))
;	(celda (caja ?caja2&~?caja)(fila ?fila)(id ?id3&~?id2&~id)(rango $?principio3 ?valor $?final3))
;	(celda (caja ?caja2)(fila ?fila2)(id ?id4&~?id3&~?id2&~id)(rango $?principio4 ?valor $?final4))
;	(not (celda (caja ?caja) (fila ~?fila2&~?fila1) (rango $? ?valor $?)))
;	?celda <- (celda (fila ?fila|?fila2)(id ?id5&~?id4&~?id3&~?id2&~id)(rango $?principio5 ?valor $?final5&:(> (+ (length$ ?principio5) (length$ ?final5)) 0)))
;	(not (celda (caja ?caja|?caja2)(id ?id6&~?id5&~?id4&~?id3&~?id2&~id)(rango $?principio6 ?valor $?final6)))
;=>
;	(modify ?celda (rango ?principio5 ?final5)(acciones $?acciones 
;	(create$ cruz-caja-fila ?celda ?valor)))
;	
;	)


;(defrule CP::cruz-caja-columna
;	(celda (caja ?caja)(columna ?columna)(id ?id)(rango $?principio ?valor $?final))
;	(celda (caja ?caja)(columna ?columna2&~?columna)(id ?id2&~?id)(rango $? principio2 ?valor $?final2))
;	(celda (caja ?caja2&~?caja)(columna ?columna)(id ?id3&~?id2&~id)(rango $?principio3 ?valor $?final3))
;	(celda (caja ?caja2)(columna ?columna2)(id ?id4&~?id3&~?id2&~id)(rango $?principio4 ?valor $?final4))
;	(not (celda (caja ?caja) (columna ~?columna2&~?columna1) (rango $? ?valor $?)))
;	?celda <- (celda (columna ?columna|?columna2)(id ?id5&~?id4&~?id3&~?id2&~id)(rango $?principio5 ?valor $?final5&:(> (+ (length$ ?principio5) (length$ ?final5)) 0)))
;	(not (celda (caja ?caja|?caja2)(id ?id6&~?id5&~?id4&~?id3&~?id2&~id)(rango $?principio6 ?valor $?final6)))
;=>
;	(modify ?celda (rango ?principio5 ?final5)(acciones $?acciones 
;	(create$ cruz-caja-columna ?celda ?valor)))
;	
;	)
...
(deffunction MAIN::resuelve-sudokus (?fichero)
 ...
 ;(resuelve-sudokus "easy50.txt") los resuelve todos casi al instante 
 
 ; (resuelve "043080250600000000000001094900004070000608000010200003820500000000000005034090710") este sudoku tarda más en resolverlo
 ; (resuelve "300200000000107000706030500070009080900020004010800050009040301000702000000008006") resuelve al instante
 ; (resuelve "380000000000400785009020300060090000800302009000040070001070500495006000000000092") resuelve al instante
 ; (resuelve "48.3............71.2.......7.5....6....2..8.............1.76...3.....4......5....") tarda un poco menos que el primero 
